  ...
  const handleMfaSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err instanceof FirebaseError && err.code === "auth/invalid-verification-code") {
        ...
        setMfaError(t("auth.twoFactor.genericError"));
      }
    } finally {
          ...
          {mfaResolver ? (
            <form className="space-y-4" onSubmit={handleMfaSubmit}>
              <div className="space-y-1">
                <h2 className="text-lg font-semibold">{t("auth.twoFactor.mfaPromptTitle")}</h2>
                <p className="text-sm text-muted-foreground">
                  {t("auth.twoFactor.mfaPromptDescription")}
                </p>
              </div>
              <div className="space-y-2">
                <Label htmlFor="mfa-code">{t("auth.twoFactor.codeLabel")}</Label>
                <Input
                  id="mfa-code"
                  inputMode="numeric"
              ...
              {mfaError && (
                <p className="text-center text-sm text-red-600">{mfaError}</p>
              )}
              ...
              >
                {isMfaLoading ? t("auth.signingIn") : t("auth.twoFactor.mfaVerifyButton")}
              </Button>
            </form>
          ) : (
            <>
