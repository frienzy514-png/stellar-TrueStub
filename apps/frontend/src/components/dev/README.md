...
- Debug/performance inspection utilities
- Anything that should never appear in a production build

## Convention

1. **Never import** anything from `dev/` in production pages, providers, or layouts.
2. If you need to use one of these components locally, do it in a dev-only route  
   ...
   const ApolloTestComponent = (await import("@/components/dev/ApolloTestComponent")).default;
   export default function Page() { return <ApolloTestComponent />; }
   ```

3. Each file in `dev/` should throw (or log a warning) if accidentally  
...
| `CacheInspector.tsx` | Inspects and displays the Apollo client cache state |
| `GraphQLDebugger.tsx` | Displays debug information for GraphQL operations |
| `QueryPerformance.tsx` | Tracks and displays GraphQL query performance metrics |
