...
interface MobilePurchaseCardProps {
  ...
  isOpen?: boolean
  onClose?: () => void
  onPurchaseNow?: (details: PurchaseDetails) => void
  onReserve?: (details: PurchaseDetails) => void
}
...
const MobilePurchaseCard = ({
  ...
  const subtotal = price * (purchaseDetails.nights || 1)
  const totalBeforeTaxes = subtotal + serviceFee + cleaningFee
  const totalPrice = totalBeforeTaxes + taxes

  const handlePurchaseNow = () => {
    if (onPurchaseNow) {
      onPurchaseNow(purchaseDetails)
    } else {
      console.log('Mobile purchase initiated:', purchaseDetails)
    }
  }

  const handleReserve = () => {
    if (onReserve) {
      onReserve(purchaseDetails)
    } else {
      console.log('Mobile reservation initiated:', purchaseDetails)
    }
  }
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="border rounded-xl p-4 bg-muted/30">
                  ...
                  <input
                    type="date"
                    value={purchaseDetails.transferInitiated}
                    onChange={(e) => setPurchaseDetails(prev => ({ ...prev, transferInitiated: e.target.value }))}
                ...
                <div className="border rounded-xl p-4 bg-muted/30">
                  ...
                  <input
                    type="date"
                    value={purchaseDetails.transferCompleted}
                    onChange={(e) => setPurchaseDetails(prev => ({ ...prev, transferCompleted: e.target.value }))}
                    className="w-full text-sm font-medium bg-transparent border-none p-0 focus:outline-none"
                    placeholder="Add date"
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <Button
                      ...
                      onClick={() => setPurchaseDetails(prev => ({
                        ...prev,
              ...
              {showPriceBreakdown && (
                <div className="space-y-3 pt-2">
                  <div className="flex justify-between text-sm">
                    <span>{currency}{price} × {purchaseDetails.nights || 1} night{(purchaseDetails.nights || 1) > 1 ? 's' : ''}</span>
